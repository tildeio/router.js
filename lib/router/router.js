...
/**
 * @property {RouteRecognizer} recognizer
 *           Route-recognizer instance
 *
 * @property {Object} delegate
 *           Forwards this delegate to the recognizer instance.
 *           Note: Needs to be set before a map() call!
 *
 * @property {Transition} activeTransition
 *           Currently active transition. This will be set only during a
 *           transition.
 *
 * @property {Function} didTransition({HandlerInfo[]})
 *           Will be called when transition was finalized
 *
 * @property {HandlerInfo[]} currentHandlerInfos
 *           List of entered and active HandlerInfo's
 *
 * @property {TransitionState} state
 *           Current state of the router. This is set during the finalization
 *           of a transition, but might be already set at the beginning of the
 *           transition when intermediate is set.
 *
 * @property {Function} getHandler ({String} name) -> {Object}
 *           Resolver for handler strings to objects.
 *           Note: Use this to implement a custom resolver for string handlers
 *                 to handler objects!
 *
 * @property {Object} _changedQueryParams
 *           Dictionary of changed query parameters. Only has value during
 *           queryParamsDidChange event.
 *
 * @property {String} _willChangeContextEvent
 *           Name of the change context event. This can be used to change the
 *           identifier for this event.
 *
 * @property {Function} triggerEvent({HandlerInfo[]} handlerInfos, {Boolean} ignoreFailure, {Array} args ({String} name, ...))
 *           Hook for a custom event-trigger, replacing the build in events.
 *
 */
Router.prototype = {
  ...
  /**
    ...
    This method makes sure that the parent is also referenced by
    the child when the path is "/" or "".

    Routes with the handler ending in ".index" are also referred
    to the parent, no matter what path is defined for the child.

    For Example:
     The parent is referenced by "/" and "/home" when "/home" is
     used for the child's path and "post.index" for its handler.
  ...
  */
  ...
  /**
   * Checks if a route exists by relaying request to route-recognizer
   *
   * @param {String} route
   * @returns {Boolean}
   */
...
/**
  ...
  * `reset`: reversed list of `HandlerInfo` objects in `updatedContext`

  @param {HandlerInfo[]} oldState Old state with a list of the
    handler information for the previous URL (or `[]` if this is the
    first handled transition)
  @param {HandlerInfo[]} newState New state with a list of the
    handler information for the new URL
...
/**
  ...
  @param {Router} router
  @param {Array} args arguments passed to transitionTo,
